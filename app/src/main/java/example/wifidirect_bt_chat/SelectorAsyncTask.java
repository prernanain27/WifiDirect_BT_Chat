...
public class SelectorAsyncTask extends AsyncTask<Void, Void, Void> {
    ...
    private void select() {
        ...
        while (true) {
            try {
                ...
                notifyConnectionService(Constants.MSG_SELECT_ERROR, null, null);
                break;
            }
        }
    }
    ...
    public void processSelectionKey(Selector selector, SelectionKey selKey) throws IOException {
        if (selKey.isValid() && selKey.isAcceptable()) {  // there is a connection to the server socket channel
            ...
            notifyConnectionService(Constants.MSG_NEW_CLIENT, sChannel, null);
            ...
            notifyConnectionService(Constants.MSG_FINISH_CONNECT, sChannel, null);
    ...
    public void doReadable(SocketChannel schannel){
        ...
        if( data != null ){
            ...
            notifyConnectionService(Constants.MSG_PULLIN_DATA, schannel, b);
        }
    }
    ...
    public String readData(SocketChannel sChannel) {
        ...
        try {
            ...
            if (numBytesRead == -1) {
                ...
                notifyConnectionService(Constants.MSG_BROKEN_CONN, sChannel, null);
                // sChannel.close();
            } else {
